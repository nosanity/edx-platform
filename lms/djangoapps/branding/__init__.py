...
def get_visible_courses(org=None, filter_=None, user=None):
    """
    ...
    if org:
        ...
        if not current_site_orgs or org in current_site_orgs:
            courses = CourseOverview.get_all_courses(orgs=[org], filter_=filter_, user=user)
    elif current_site_orgs:
        # Only display courses that should be displayed on this site
        courses = CourseOverview.get_all_courses(orgs=current_site_orgs, filter_=filter_, user=user)
    else:
        courses = CourseOverview.get_all_courses(filter_=filter_, user=user)
